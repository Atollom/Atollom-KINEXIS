      ...
      {/* Input row */}
       ...
       {files.length > 0 && (
         <div className="flex gap-2 pb-3 overflow-x-auto">
           {files.map((file, i) => (
             <div key={i} className="flex items-center gap-2 bg-white/[0.06] rounded-lg px-3 py-1.5">
               <span className="material-symbols-outlined text-sm text-on-surface-variant">
                 {file.type.includes('pdf') ? 'description' : 'table_chart'}
               </span>
               <span className="text-xs text-on-surface truncate max-w-[150px]">{file.name}</span>
               <button 
                 onClick={() => setFiles(prev => prev.filter((_, idx) => idx !== i))}
                 className="ml-1 p-0.5 hover:bg-white/[0.06] rounded"
               >
       ...
       >
         ...
         <button
           ...
           disabled={disabled}
           className="flex-shrink-0 mb-0.5 p-2 hover:bg-white/[0.06] rounded-lg transition-all"
           aria-label="Subir archivo"
           title="Subir PDF, Excel o CSV"
         >
