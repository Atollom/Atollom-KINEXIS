import { Suspense } from 'react'
import { PageHeader } from '@/components/ui/PageHeader'
import { PageSkeleton } from '@/components/ui/PageSkeleton'
import { ShoppingCart } from 'lucide-react'

export const metadata = {
  title: 'Órdenes de Compra | KINEXIS',
  description: 'Creación y seguimiento de órdenes de compra',
}

export default async function PurchaseOrdersPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Órdenes de Compra"
        description="Creación y seguimiento de órdenes de compra"
        badge="#30"
      />
      <Suspense fallback={<PageSkeleton />}>
        <div className="glassmorphism p-12 flex flex-col items-center text-center gap-4">
          <div className="w-16 h-16 rounded-full bg-[#CCFF00]/10 flex items-center justify-center">
            <ShoppingCart className="w-8 h-8 text-[#CCFF00]/70" />
          </div>
          <h3 className="text-lg font-semibold text-white/80">Agente #30 — Órdenes de Compra</h3>
          <p className="text-sm text-white/40 max-w-md">
            Generación automática de órdenes de compra basada en punto de reorden disponible en Fase 2.
          </p>
        </div>
      </Suspense>
    </div>
  )
}
